...
def perform_search(query_paragraph, paragraphs, embeddings, model_name='roberta-base-nli-stsb-mean-tokens', k=3):
    ...
    index = faiss.IndexFlatL2(d)  # L2 distance is used
    index.add(embeddings)
    
    # Convert query_embedding to float32 (required by Faiss)
    query_embedding = np.array(query_embedding[0], dtype=np.float32).reshape(1, -1)
...
# Load the embeddings
paragraphs, embeddings = load_embeddings('chunked_embeddings.json')
